...
1. We integrated PaddleOCR as a specialized fallback. While VLMs are incredible at layout understanding, they can sometimes hallucinate exact alphanumeric strings (like complex tracking numbers).
2. We use OCR as well in case of VLM got down or got failed to extract the data.
3. We can use simple Agentic Document Extractor to extract the data from the document using Traditional OCR such as PaddleOCR and then develop AI Agent to understand the data extracted by OCR.
...
The VLM is strictly relegated to _data extraction_ (Image -> JSON). Once the orchestrator returns the structured, strictly-typed Pydantic model (`ExtractionResponse`), the AI's job is over.
...
- _Math checks:_ `sum(line_items) == subtotal`
- _Cross-document checks:_ `invoice.total_gross_weight == packing_list.total_gross_weight`
- _Completeness checks:_ `invoice.date is not None`
...
1. **API Rate Limiting (429 Errors):** We are currently executing concurrent requests against a single Gemini API key. Processing ~7 documents a minute continuously would rapidly exhaust the free-tier RPM/TPM limits, causing cascading failures.
   ...
   - Also we can use multiple ai providers to distribute the load and avoid rate limiting, for example we can use Gemini API for simple extractions and use Claude API for complex extractions or multiple api keys from the same provider.

That is an excellent architectural strategy. In production systems, relying on a single API provider is a single point of failure. This addition highlights two very advanced and realistic concepts:

- Cost & Capability Routing: Using a cheaper/faster model (like Gemini Flash) for simple tasks and a heavier model (like Claude 3.5 Sonnet) for complex edge cases is a very common optimization.
- Key Rotation/Load Balancing: Distributing requests across multiple API keys or accounts is a standard way to bypass strict RPM (Requests Per Minute) limits before enterprise agreements are in place.
...
Extraction accuracy measures: _Did the model correctly pull the data from the image?_
...
Compliance accuracy measures: _Did the engine apply the business rules correctly?_
